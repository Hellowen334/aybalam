...
# User Playlists Database Helpers
async def get_playlist_names(user_id: int) -> list:
    playlist = await playlistdb.find_one({"user_id": user_id})
    if not playlist:
        return []
    return playlist.get("playlist", [])

async def is_playlist_exist(user_id: int, videoid: str) -> bool:
...
async def add_playlist(user_id: int, videoid: str, title: str, duration: str) -> bool:
    is_exist = await is_playlist_exist(user_id, videoid)
    if is_exist:
        return False
    await playlistdb.update_one(
        {"user_id": user_id},
        {"$push": {"playlist": {"title": title, "videoid": videoid, "duration": duration}}},
        upsert=True,
    )
    return True

async def remove_playlist(user_id: int, videoid: str) -> bool:
    is_exist = await is_playlist_exist(user_id, videoid)
    if not is_exist:
        return False
    await playlistdb.update_one(
        {"user_id": user_id},
        {"$pull": {"playlist": {"videoid": videoid}}},
    )
    return True
