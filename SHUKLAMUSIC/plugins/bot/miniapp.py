# -----------------------------------------------
# 🔸 Aybalam Music — Mini App (Web App) Handler
# 🔹 Receives data from the Aybalam Music mini app
...
_POWERED = (
    f"✦ ᴘᴏᴡᴇʀᴇᴅ ʙʏ » <a href='https://t.me/{config.OWNER_USERNAME}'>"
    f"{config.BOT_NAME}</a>"
)
...
async def _download_and_send(chat_id: int, vidid: str, status_msg):
    ...
    try:
        ...
        await app.send_audio(
            chat_id=chat_id,
            audio=out_file,
            caption=f"🎵 <b>{config.BOT_NAME} — ᴍɪɴɪ ᴀᴘᴘ ᴅᴏᴡɴʟᴏᴀᴅ</b>\n\n{_POWERED}",
        )
    except Exception:
        try:
            await status_msg.edit_text("❌ ᴅᴏᴡɴʟᴏᴀᴅ ꜰᴀɪʟᴇᴅ. ᴛʀʏ ᴀɢᴀɪɴ ʟᴀᴛᴇʀ.")
...
@app.on_message(_has_web_app_data & filters.private & ~config.BANNED_USERS)
async def handle_mini_app_data(client, message: Message):
    """
    Receives data submitted from the Aybalam Music mini app and
