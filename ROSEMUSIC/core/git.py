...
def git():
    ...
    if not UPSTREAM_REPO:
        LOGGER(__name__).warning("No UPSTREAM_REPO configured — skipping git updates.")
        return
    try:
        repo = Repo()
        ...
        try:
            ...
            LOGGER(__name__).warning(f"Failed to fetch from upstream: {e}")
            return
        ...
        try:
            ...
            LOGGER(__name__).warning(f"Failed to pull updates: {e}")
