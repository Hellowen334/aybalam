...
async def sudoers_list(client, message: Message):
    keyboard = [[InlineKeyboardButton("๏ ʏᴇᴛᴋİʟİ ʟİsᴛᴇsİɴİ ɢÖʀ ๏", callback_data="check_sudo_list")]]
    ...
    await message.reply_video(video="https://telegra.ph/file/7fceefa2fb3e21f5fd84e.mp4", caption="**» ᴀŞᴀĞıᴅᴀᴋİ ʙᴜᴛᴏɴᴅᴀɴ ʏᴇᴛᴋİʟİ ʟİsᴛᴇsİɴİ ɢÖʀᴇʙİʟİʀsİɴİᴢ.**\n\n**» ɴᴏᴛ:** Sadece yetkililer görebilir. ", reply_markup=reply_markups)
...
async def check_sudo_list(client, callback_query: CallbackQuery):
    ...
    if callback_query.from_user.id not in SUDOERS:
        return await callback_query.answer("Bu listeyi sadece yetkililer görebilir! 🖕😎😂", show_alert=True)
    else:
        user = await app.get_users(OWNER_ID)

        user_mention = (user.first_name if not user.mention else user.mention)
        caption = f"**˹ʙᴏᴛ ʏÖɴᴇᴛİᴄİʟᴇʀİ ʟİsᴛᴇsİ˼**\n\n**🌹ᴋᴜʀᴜᴄᴜ** ➥ {user_mention}\n\n"

        keyboard.append([InlineKeyboardButton("๏ ᴋᴜʀᴜᴄᴜʏᴜ ɢÖʀ ๏", url=f"tg://openmessage?user_id={OWNER_ID}")])
        ...
        for user_id in SUDOERS:
            if user_id != OWNER_ID:
                try:
                    ...
                    button_text = f"๏ ʏᴇᴛᴋİʟİ {count} ɢÖʀ ๏ "
        ...
        keyboard.append([InlineKeyboardButton("๏ ɢᴇʀİ ๏", callback_data="back_to_main_menu")])
...
async def back_to_main_menu(client, callback_query: CallbackQuery):
    ...
    await callback_query.message.edit_caption(caption="**» ᴀŞᴀĞıᴅᴀᴋİ ʙᴜᴛᴏɴᴅᴀɴ ʏᴇᴛᴋİʟİ ʟİsᴛᴇsİɴİ ɢÖʀᴇʙİʟİʀsİɴİᴢ.**\n\n**» ɴᴏᴛ:** Sadece yetkililer görebilir. ", reply_markup=reply_markupes)
