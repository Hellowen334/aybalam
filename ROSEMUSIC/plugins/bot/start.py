...
# ================================
_ROSE_THUMB = "https://telegra.ph/file/b4a3a6962f9157ad7fdb3.jpg"
ROSE_PICS = [_ROSE_THUMB] * 9

ROSE_BOT_NAME = config.BOT_NAME
ROSE_BOT_USERNAME = f"@{config.BOT_USERNAME}"
ROSE_OWNER = f"@{config.OWNER_USERNAME}"
ROSE_SUPPORT_LINK = config.SUPPORT_CHAT
ROSE_UPDATES_LINK = config.SUPPORT_CHANNEL
ROSE_POWERED_BY = "AყႦαʅαɱ Mυʂιƈ"
        ...
        if name in {"chatfight", "wordplay", "wordgame"}:
            ...
            try:
                ...
                if dl_type == "v":
                    ...
                    for candidate in [tmp_file, tmp_file + ".mp4"] + _glob.glob(tmp_file + "*"):
                    ...
                    import glob as _glob
                    for candidate in [tmp_file, tmp_file + ".mp3"] + _glob.glob(tmp_file + "*"):
                        if os.path.exists(candidate) and os.path.getsize(candidate) > 10_000:
                            os.replace(candidate, out_file)
                            downloaded = True
                            break

                if not downloaded or not (os.path.exists(out_file) and os.path.getsize(out_file) > 0):
